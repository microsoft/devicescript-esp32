...
void jd_rx_completed(int dataLeft) {
    ...
    LOG("rx cmpl; sz=%d decl=%d", txSize, declaredSize);
    ...
    if (crc != frame->crc) {
        LOG("crc=%x sz=%d fl=%d dev=%x %x", frame->crc, frame->size, frame->flags,
              (uint32_t)(frame->device_identifier >> 32),
              (uint32_t)(frame->device_identifier >> 0));
        ERROR("crc mismatch");
        jd_diagnostics.bus_uart_error++;
        return;
    }
